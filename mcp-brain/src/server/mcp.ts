...
Provide identity in 'agent' field, format content as **What**/**Why**/**Where**/**Learned**`,
			...
			},
			{
				...
				description:
					"Save user prompt to persistent memory for session context.",
				inputSchema: {
					type: "object",
					properties: {
						project: { type: "string" },
			...
			},
			{
				...
				description:
					"Extract structured learnings from text output automatically.",
				inputSchema: {
					type: "object",
					properties: {
						project: { type: "string" },
			...
			},
			{
				...
				description:
					"Suggest a stable topic_key for grouping related memories.",
				inputSchema: {
					type: "object",
					properties: {
						title: { type: "string" },
			...
			},
			{
				...
				description:
					"Save end-of-session summary with goal, discoveries, and next steps.",
				inputSchema: {
					type: "object",
					properties: {
						sessionId: { type: "string" },
			...
			},
			{
				name: "mem_judge",
				description:
					"Record verdict on a memory conflict surfaced by mem_save. Provide judgment_id and relation.",
				inputSchema: {
					type: "object",
					properties: {
			...
			},
			{
				...
				description:
					"List available scaffolding templates for agents, rules, or workflows.",
				inputSchema: {
					type: "object",
					properties: {
						tool: {
							type: "string",
							description:
								"Filtrar por tool: antigravity | opencode | universal",
						},
						type: {
							type: "string",
							description: "Filtrar por type: rule | workflow | agent",
						},
					},
				},
			},
			{
				name: "scaffold_file",
				description:
					"Generate a scaffold file from a stored template with provided variables.",
				inputSchema: {
					type: "object",
					properties: {
						template_id: {
							type: "string",
							description:
								"ID del template (usar scaffold_list_templates para ver IDs disponibles)",
						},
						variables: {
							type: "object",
							description: "Variables para rellenar el template (clave: valor)",
						},
					},
			...
			},
			{
				...
				description:
					"Get core directives (project rules and personality) for a project.",
				inputSchema: {
					type: "object",
					properties: {
						project: {
							type: "string",
							description: "Project name (default: lallamaollama)",
						},
					},
				},
			},
		...
		tools.push({
			...
			description:
				"Check your compliance status with the shared brain audit system.",
			inputSchema: {
				type: "object",
				properties: {
					agent: {
						type: "string",
						description:
							"Your identity (optional — defaults to auto-detected agent)",
					},
				},
			},
		});
	...
	mcpServer.setRequestHandler(CallToolRequestSchema, async (request) => {
		...
		try {
			switch (name) {
				...
				case "mem_save_prompt": {
					...
					response = {
						content: [
							{
								type: "text",
								text: `Prompt saved successfully. ID: ${memory.memory.id}`,
							},
						],
					};
					break;
				...
				case "mem_capture_passive": {
					const _project = normalizeProject(args?.project as string);
					...
					if (learnings.length === 0) {
						response = {
							content: [{ type: "text", text: "No key learnings found." }],
						};
						break;
					}
					...
					response = {
						content: [
							{ type: "text", text: `Captured ${learnings.length} learnings.` },
						],
					};
					break;
				}
				case "mem_suggest_topic_key": {
					...
					try {
						...
						const llmResult = await generate(
							model,
							`Given the title "${title}" and type "${type}", suggest a short stable topic key (2-5 words, lowercase, hyphen-separated) for grouping related memories. Return only the key.`,
						...
						if (cleaned.length > 0) {
							response = {
								content: [{ type: "text", text: `${type}/${cleaned}` }],
							};
							break;
						}
					} catch {
					...
					response = {
						content: [{ type: "text", text: `${type}/${fallbackSlug}` }],
					};
					break;
				}
				case "mem_update": {
					...
					response = {
						content: [
							{
								type: "text",
								text: success ? "Memory updated." : "Memory not found.",
							},
						],
					};
					break;
				...
				case "mem_delete": {
					const success = await memories.deleteMemory(
						dbService,
						args?.id as string,
					);
					response = {
						content: [
							{
								type: "text",
								text: success ? "Memory deleted." : "Memory not found.",
							},
						],
					};
					break;
				...
				case "mem_search": {
					...
					const mems = await memories.searchMemories(
						...
						project,
						(args?.mode as "lexical" | "semantic" | "hybrid" | undefined) ||
							"hybrid",
						(args?.limit as number) || 10,
					);
					response = {
						content: [{ type: "text", text: JSON.stringify(mems) }],
					};
					break;
				}
				case "mem_context": {
				...
				case "mem_get_observation": {
					const memory = await memories.getMemory(
						dbService,
						args?.id as string,
					);
					response = {
						content: [
							{
								type: "text",
								text: memory ? JSON.stringify(memory) : "Not found",
							},
						],
					};
					break;
					...
					if (args?.project) {
						...
						response = {
							content: [
								{
									type: "text",
									text: `Active project set to: ${currentProject}`,
								},
							],
						};
					} else {
				...
				case "mem_session_start": {
					...
					response = {
						content: [{ type: "text", text: `Session started. ID: ${id}` }],
					};
					break;
				}
				...
				case "mem_session_summary": {
					const summary = await sessions.getSessionSummary(
						dbService,
						args?.sessionId as string,
					);
					response = {
				...
				case "mem_judge": {
					...
					if (
						!VALID_RELATIONS.includes(
							relation as (typeof VALID_RELATIONS)[number],
						)
					) {
						response = {
							content: [
								{
									type: "text",
									text: `Error: Invalid relation "${relation}". Valid values: ${VALID_RELATIONS.join(", ")}`,
								},
							],
							isError: true,
						};
				...
				case "mem_get_directives": {
					const project = normalizeProject(
						(args?.project as string) || "lallamaollama",
					);
				...
				case "mem_my_compliance": {
					...
					const compliance = await audit.getAgentCompliance(
						dbService,
						targetAgent,
						24,
					);
					response = {
						content: [{ type: "text", text: JSON.stringify(compliance) }],
					};
					break;
				}
				...
				case "scaffold_file": {
					const tpl = await templates.getTemplate(
						dbService,
					...
					if (!tpl) {
						response = {
							content: [
								{
									type: "text",
									text: `Template "${args?.template_id}" not found. Use scaffold_list_templates to see available templates.`,
								},
							],
							isError: true,
						};
				...
				default:
					response = {
						content: [
							{
								type: "text",
								text: `Tool ${name} implemented but handler missing.`,
							},
						],
					};
			}
		...
		if (response) {
			...
			audit
				...
				.catch((err: unknown) =>
					log.error({ err }, "Audit error logging tool call"),
				);
			// Cleanup oportunista: 1 de cada 100 llamadas elimina registros >30 días
			if (Math.random() < 0.01) {
				dbService
					.getDb()
					.run(`DELETE FROM mcp_audit_log WHERE timestamp < ?`, [
