		...
		try {
			...
			if (!result.success) {
				log.warn(
					...
					"Container start returned not-found",
				);
				return res
					.status(404)
			...
			log.error(
				{ container: ollamaContainer, message },
	...
	router.post("/api/ollama/stop", authMiddleware, async (_req, res) => {
		...
		try {
			...
			if (!result.success) {
				log.warn(
					{ container: ollamaContainer },
					"Container stop returned not-found",
				);
				return res
					.status(404)
					.json({ error: { message: result.message, type: "not_found" } });
	...
	router.post("/api/ollama/restart", authMiddleware, async (_req, res) => {
		...
		try {
			...
			if (!result.success) {
				log.warn(
					{ container: ollamaContainer },
					"Container restart returned not-found",
				);
				return res
					.status(404)
					.json({ error: { message: result.message, type: "not_found" } });
		...
		try {
			...
			if (!result.success) {
				log.warn(
					...
					"Brain start returned not-found",
				);
				return res
					.status(404)
					.json({ error: { message: result.message, type: "not_found" } });
