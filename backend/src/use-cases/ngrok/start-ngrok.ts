...
export class StartNgrokUseCase {
  ...
  async execute() {
    ...
    if (!container) {
      return { success: false as const, message: "Contenedor ngrok no encontrado. Verifica docker-compose." };
    }
    if (await this.dockerRepo.isRunning(this.containerName)) {
      return { success: true as const, message: "Ngrok ya está corriendo", running: true };
    }
