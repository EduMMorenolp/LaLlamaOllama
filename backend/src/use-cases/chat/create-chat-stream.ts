...
export class CreateChatStreamUseCase {
	...
	async execute(input: ChatRequest, res: Response): Promise<void> {
		...
		streamResponse.data.on("end", () => {
			...
			const tokensPerSec =
				completionTokens > 0 ? (completionTokens / totalDurationMs) * 1000 : 0;
			...
			if (ttftMs > 0) {
				...
				if (!Array.isArray(stats.tokensPerSecHistor))
					stats.tokensPerSecHistor = [];
				stats.tokensPerSecHistor.push(tokensPerSec);
				if (stats.tokensPerSecHistor.length > 100)
					stats.tokensPerSecHistor.shift();
			}
