...
export function saveMessageToFavorites(userId: string, chatId: string, messageRole: string, messageContent: string, messageTimestamp?: string): boolean {
  const db = getDb();
  try {
    db.prepare(
...
export function unsaveMessage(userId: string, chatId: string, messageContent: string): boolean {
  ...
  try {
    db.prepare(
      `DELETE FROM saved_messages WHERE userId = ? AND chatId = ? AND messageContent = ?`
    ).run(userId, chatId, messageContent);
    return true;
  } catch {
    return false;
  }
...
export function listSavedMessages(userId: string, limit = 50): SavedMessage[] {
  const db = getDb();
  return db.prepare(
    "SELECT * FROM saved_messages WHERE userId = ? ORDER BY created_at DESC LIMIT ?"
...
export function isMessageSaved(userId: string, chatId: string, messageContent: string): boolean {
  ...
  const row = db.prepare(
    "SELECT id FROM saved_messages WHERE userId = ? AND chatId = ? AND messageContent = ?"
